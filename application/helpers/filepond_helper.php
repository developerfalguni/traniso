...
function remove_transfer_directory($path, $id) {
    ...
    remove_directory($path . DIRECTORY_SEPARATOR . $id . DIRECTORY_SEPARATOR . $CI->_variants_dir);
...
function store_transfer($path, $transfer) {
    ...
    if ($transfer->getMetadata()) {
        $CI =& get_instance();
        write_file($path, @json_encode($transfer->getMetadata()), $CI->_metadata_filename);
    }
    ...
    if (count($transfer->getFiles()) > 1) {
        ...
        $variants = $path . DIRECTORY_SEPARATOR . $CI->_variants_dir;
...
function get_transfer($path, $id) {
    ...
    $metadata = get_file($path, $CI->_metadata_filename);

    $variants = get_files($path . DIRECTORY_SEPARATOR . $CI->_variants_dir, '*.*');
...
function route_form_post($entries, $routes) {
    ...
    foreach ($entries as $entry) {
        ...
        call_user_func(array('save', $routes[$post->getFormat()]), $post->getValues());
    }
} 
