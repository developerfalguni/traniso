...
<div class="row">
	<div class="col-md-4">
		<?php echo start_panel('Attach Documents', '', 'nopadding', '<div class="buttons">' . 
			anchor('master/staff/edit/'.$staff_id['staff_id'], 'Party Ledger', 'class="btn btn-sm btn-info"') .
		'</div>'); ?>
		...
		<?php foreach ($documents as $doc) {
			echo '<tr>
			<td>' . $doc['date'] . '</td>
			<td>' . anchor($this->_clspath.$this->_class.'/index/'.$doc['staff_id'].'/'.$doc['id'], $doc['name']) . '</td>
...
function deleteDocument(id) {
	$("a#DeleteUrl").attr("href", '<?php echo base_url($this->_clspath.$this->_class.'/detach/'.$staff_id['staff_id']) ?>/'+id);
	$("#modal-delete").modal();
}
</script>