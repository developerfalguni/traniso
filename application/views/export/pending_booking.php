...
<div id="FixedToolbar">
	...
	<tr>
		...
		<td>
			<div class="nowrap">
				...
				<div class="btn-group">
					<button data-toggle="dropdown" class="btn btn-warning dropdown-toggle"><?php echo (isset($parsed_search['cargo']) ? '<i class="icon-filter4"></i>' : '') ?> Cargo <span class="caret"></span></button>
					<ul class="dropdown-menu nav-menu-scroll" id="FilterCargo">
	...
	<tr>
		...
		<th>Cont.</th>
		<th>Shipping Line</th>
		<th>Vessel / <span class="orange">Cutoff</span> - <span class="green">ETD</span></th>
		<th>POL</th>
		<th>POD</th>
		<th>CFS</th>
	</tr>
...
foreach ($export_details as $r) {
	...
	<td class="tiny blue">' . $r['bl_no'] . ' / ' . $r['bl_date'] . '</td>
	...
	<td class="tiny">' . character_limiter($r['cfs_name']) . '</td>
	</tr>';
}
?>
</tbody>
...
if (count($filter['party']) > 0) {
	...
	foreach ($filter['party'] as $k => $v)
		echo '$("ul#FilterParty").append("<li><a href=\"javascript: filter(\'party: ' . $k . '\')\">' . urldecode($k) . '</a></li>");';
}
if (count($filter['cargo']) > 0) {
	ksort($filter['cargo']);
	foreach ($filter['cargo'] as $k => $v)
		echo '$("ul#FilterCargo").append("<li><a href=\"javascript: filter(\'cargo:' . $k . '\')\">' . $k . '</a></li>");';
}
