...
<div class="card card-default">
	...
	<div class="card-header pt-0">
		<div class="row d-none" id="jobInfoButtons">
			<div class="col-md-10">
				<button type="button" data-toggle="modal" class="btn btn-info ml-2" onclick="javascript: updateContainers()" id="containerButton"> <i class="fa fa-truck"></i> Update Containers</button>
				<button type="button" class="btn btn-info ml-2" onclick="javascript: updateCostsheet()" id="costsheetButton"> <i class="fa fa-rupee-sign"></i> Update Costsheet</button>
				
				<button type="button" data-toggle="modal" class="btn btn-info ml-2" onclick="javascript: attachDocuments()" id="documentButton"> <i class="fa fa-upload"></i> Upload Documents</button>

				<button type="button" class="btn btn-info ml-2" onclick="javascript: fileCover()" id="filecoverButton"> <i class="fa fa-file"></i> File Cover</button>
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="float-left pt-2">
					<button type="button" class="btn btn-primary" id="Joblist" onclick="updateJobList()"><i class="fa fa-list"></i> Job List</button>
					<button type="button" class="btn btn-success" id="Add" onclick="updateJob('0')"><i class="fa fa-plus"></i> Add</button>
					<button type="button" class="btn btn-success d-none" id="btnForward"  onclick="updateForwarding()"><i class="fa fa-pencil-alt "></i>Forwarding</button>
				</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#cha_name').kaabar_autocomplete({source: '<?php echo site_url('/master/operation/vendor/ajax/CHA') ?>', alertText: 'CHA name'});
	    ...
	    $('input[type=checkbox][id=Forwarding').change(function() {
	        if ($(this).is(':checked')) {
    ...
    {   
        	...
        	$.ajax({
                ...
                success:function(response) {
                	...
                	else
                	{
						...
						$('input[type=checkbox][id=Clearing').prop('checked', false);
						$('input[type=checkbox][id=Forwarding').prop('checked', false);
						$('input[type=checkbox][id=Transportation').prop('checked', false);

						$.each(response.job.sub_type, function(index, value) {
							if(value === 'Transportation')
					    		$('input[type=checkbox][id=Transportation').prop('checked', true);
					    	if(value === 'Clearing')
					    		$('input[type=checkbox][id=Clearing').prop('checked', true);
					    	if(value === 'Forwarding'){
					    		$('input[type=checkbox][id=Forwarding').prop('checked', true);
					    		$('#btnForward').removeClass('d-none');
					    	}
					    });
        ...
        else
        {
			...
			$("#jobInfoButtons").addClass('d-none');
			$("#btnForward").addClass('d-none');
		}
...
<script type="text/javascript">
    ...
    $('#modal-forwarder').on('hidden.bs.modal', function () {
        $(this).find('#updateForwarder').html('');
    })

    function fileCover()
    {   
    	var id = $("input[name=id]").val();
    	var win = window.open('<?php echo base_url($this->_clspath.$this->_class) ?>/filecover/'+id, '_blank');
		if (win) {
    ...
    {   
    	...
    	if(id > 0){
        	$.ajax({
                url: '<?php echo site_url() ?>export/container/index/'+id,
                type: 'post',
                dataType: 'json',
                beforeSend: function(){
		            $("#loader").show();
                ...
                success:function(response) {
                    ...
                    <?php //if(Auth::hasPermission(Auth::READ, ['resource/location'])){ ?>
						my_html += '<form action="<?php echo base_url('export/container/edit/') ?>'+id+'" method="post" id="frmUpdateContainers" enctype="multipart/form-data">';
							my_html += '<div class="modal-body p-0">';
								...
								my_html += '<input type="hidden" name="packages" id="c_packages" value="'+response.jobs.packages+'">';
								my_html += '<input type="hidden" name="gross_weight" id="c_gross_weight" value="'+response.jobs.gross_weight+'">';
								my_html += '<input type="hidden" name="net_weight" id="c_net_weight" value="'+response.jobs.net_weight+'">';
							        ...
							        my_html += '<table class="table table-sm table-bordered DataEntry">';
							            my_html += '<thead>';
							            	my_html += '<tr>';
							                    ...
							                    my_html += '<th class="align-middle">No Of Pkgs</th>';
							                    my_html += '<th class="align-middle">Gross Weight</th>';
							                    my_html += '<th class="align-middle">Net Weight</th>';
										...
										if(response.rows.length){
										    var result = JSON.parse(JSON.stringify(response.rows));
										    var $i;
										    var total_cntr_packages = 0;
										    var total_cntr_gross = 0;
										    var total_cntr_net = 0;
											    ...
											    for(var i=0;i < result.length;i++){
											        ...
											        if(item.id > 0) {
											        	...
											        	my_html += '<tr>';
														    ...
														    my_html += '<td class="align-middle">';
														    	my_html += '<input type="text" class="form-control form-control-sm cntr_packages" name="cntr_packages['+item.id+']" value="'+item.cntr_packages+'" onkeypress="return isNumberDot(event)">';
														    my_html += '</td>';
														    my_html += '<td class="align-middle">';
														    	my_html += '<input type="text" class="form-control form-control-sm cntr_gross_weight" name="cntr_gross_weight['+item.id+']" value="'+item.cntr_gross_weight+'" onkeypress="return isNumberDot(event)">';
														    my_html += '</td>';
														    my_html += '<td class="align-middle">';
														    	my_html += '<input type="text" class="form-control form-control-sm cntr_net_weight" name="cntr_net_weight['+item.id+']" value="'+item.cntr_net_weight+'" onkeypress="return isNumberDot(event)">';
														    my_html += '</td>';

														    my_html += '<td class="align-middle text-center">'+item.delete_btn;+'</td>';
														my_html += '</tr>';

														total_cntr_packages += parseFloat(item.cntr_packages);
														total_cntr_gross += parseFloat(item.cntr_gross_weight);
														total_cntr_net += parseFloat(item.cntr_net_weight);
										...
										my_html += '<tr class="TemplateRow '+dnone+'">';
										    ...
										    my_html += '<td class="align-middle"><input type="text" class="form-control form-control-sm cntr_packages" onkeypress="return isNumberDot(event)" name="new_cntr_packages[]"></td>';
										    my_html += '<td class="align-middle"><input type="text" class="form-control form-control-sm cntr_gross_weight" onkeypress="return isNumberDot(event)" name="new_cntr_gross_weight[]"></td>';
										    my_html += '<td class="align-middle"><input type="text" class="form-control form-control-sm cntr_net_weight" onkeypress="return isNumberDot(event)" name="new_cntr_net_weight[]"></td>';
										...
										my_html += '<tfoot>';
											my_html += '</tr>';
												my_html += '<td colspan="12" class="text-right"> Total :</td>';
												my_html += '<td class="text-center">'+total_cntr_packages+'</td>';
												my_html += '<td class="text-center">'+total_cntr_gross+'</td>';
												my_html += '<td class="text-center">'+total_cntr_net+'</td>';
												my_html += '<td></td>';
											my_html += '</tr>';
										my_html += '</tfoot>'; 
							        my_html += '</table>';
				    ...
				    $('#modal-containers').find('.TransporterName').kaabar_autocomplete_full({source: '<?php echo site_url('/master/operation/vendor/ajax/Transporter') ?>'});
    ...
    function updateForwarding(id = null)
    {   
    	...
    	if(id > 0){
        	$.ajax({
                url: '<?php echo site_url() ?>export/forwarder/index/'+id,
                type: 'post',
                dataType: 'json',
                beforeSend: function(){
		            $("#loader").show();
                ...
                success:function(response) {
                	...
                	var my_html = '';    
                    <?php //if(Auth::hasPermission(Auth::READ, ['resource/location'])){ ?>
						my_html += '<form action="<?php echo base_url('export/Forwarder/edit/') ?>'+id+'" method="post" id="frmUpdateForwarder" enctype="multipart/form-data">';
							my_html += '<div class="modal-body p-0">';
								...
								my_html += '<div class="row col-md-12 m-1">';
									my_html += '<div class="col-md-4">';
										my_html += '<label class="control-label text-center">BL Type</label></br>';
									my_html += '</div>';
									my_html += '<div class="col-md-4">';
										my_html += '<div class="form-group">';
											my_html += '<input type="checkbox" id="MBL" name="bl_type[MBL]" value="MBL" >';
											my_html += '<label> MBL</label></br>';
										my_html += '</div>';
									my_html += '</div>';
									my_html += '<div class="col-md-4">';
										my_html += '<div class="form-group">';
											my_html += '<input type="checkbox" id="HBL" name="bl_type[HBL]" value="HBL" >';
											my_html += '<label> HBL</label>';
										my_html += '</div>';
									my_html += '</div>';
								my_html += '</div>';
								...
								my_html += '<div class="row col-md-12 m-1">';
									my_html += '<div class="col-md-3">';
										my_html += '<div class="form-group">';
											my_html += '<label class="control-label">Pricing Detail</label>';
												my_html += '<input type="text" class="form-control form-control-sm" name="pricing_detail" value="'+response.jobs.pricing_detail+'">';
										my_html += '</div>';
									my_html += '</div>';
									my_html += '<div class="col-md-3">';
										my_html += '<div class="form-group">';
											my_html += '<label class="control-label">Contact Party</label>';
											my_html += '<input type="text" class="form-control form-control-sm" name="contact_party" value="'+response.jobs.contact_party+'">';
										my_html += '</div>';
									my_html += '</div>';
									my_html += '<div class="col-md-3">';
										my_html += '<div class="form-group">';
											my_html += '<label class="control-label">Booking Type</label>';
											my_html += response.jobs.booking_type_dd;
										my_html += '</div>';
									my_html += '</div>';
									my_html += '<div class="col-md-3">';
										my_html += '<div class="form-group">';
											my_html += '<label class="control-label">Overseas Agent</label>';
											my_html += '<input type="text" class="form-control form-control-sm" name="overseas_agent" id="overseas_agent" value="'+response.jobs.overseas_agent+'">';
										my_html += '</div>';
									my_html += '</div>';
								my_html += '</div>';
								...
								my_html += '<div class="row col-md-12 m-1">';
									my_html += '<div class="col-md-6">';
										my_html += '<div class="row d-none" id="mbl_div">';
											my_html += '<div class="col-md-4">';
												my_html += '<div class="form-group">';
													my_html += '<label class="control-label">MBL No</label>';
														my_html += '<input type="text" class="form-control form-control-sm" name="mbl_no" value="'+response.jobs.mbl_no+'">';
												my_html += '</div>';
											my_html += '</div>';
											my_html += '<div class="col-md-4">';
												my_html += '<div class="form-group">';
													my_html += '<label class="control-label">MBL Date</label>';
													my_html += '<div class="input-group input-group-sm">';
														my_html += '<input type="text" class="form-control DatePicker" name="mbl_date" value="'+response.jobs.mbl_date+'">';
														my_html += '<div class="input-group-append">';
															my_html += '<div class="input-group-text"><i class="icon-calendar"></i></div>';
														my_html += '</div>';
													my_html += '</div>';
											...
											my_html += '<div class="col-md-4">';
												my_html += '<div class="form-group">';
													my_html += '<label class="control-label">HBL type</label>';
													my_html += response.jobs.mbl_type_dd;
												my_html += '</div>';
											my_html += '</div>';
										my_html += '</div>';
									my_html += '</div>';
									my_html += '<div class="col-md-6">';
										my_html += '<div class="row d-none" id="hbl_div">';
											my_html += '<div class="col-md-4">';
												my_html += '<div class="form-group">';
													my_html += '<label class="control-label">HBL No</label>';
														my_html += '<input type="text" class="form-control form-control-sm" name="hbl_no" value="'+response.jobs.hbl_no+'">';
												my_html += '</div>';
											my_html += '</div>';
											my_html += '<div class="col-md-4">';
												my_html += '<div class="form-group">';
													my_html += '<label class="control-label">HBL Date</label>';
													my_html += '<div class="input-group input-group-sm">';
														my_html += '<input type="text" class="form-control DatePicker" name="hbl_date" value="'+response.jobs.hbl_date+'">';
														my_html += '<div class="input-group-append">';
															my_html += '<div class="input-group-text"><i class="icon-calendar"></i></div>';
														my_html += '</div>';
													my_html += '</div>';
											...
											my_html += '<div class="col-md-4">';
												my_html += '<div class="form-group">';
													my_html += '<label class="control-label">MBL type</label>';
													my_html += response.jobs.hbl_type_dd;
												my_html += '</div>';
											my_html += '</div>';
										my_html += '</div>';
									my_html += '</div>';
								...
								my_html += '<div class="row col-md-12 m-1">';
									my_html += '<div class="col-md-6">';
										my_html += '<div class="form-group">';
											my_html += '<label class="control-label">Buying</label>';
											my_html += '<div class="input-group input-group-sm">';
												my_html += '<input type="text" class="form-control form-control-sm" name="fwd_buy" value="'+response.jobs.fwd_buy+'">';
											my_html += '</div>';
										my_html += '</div>';
									my_html += '</div>';
									my_html += '<div class="col-md-6">';
										my_html += '<div class="form-group">';
											my_html += '<label class="control-label">Selling</label>';
											my_html += '<div class="input-group input-group-sm">';
												my_html += '<input type="text" class="form-control form-control-sm" name="fwd_sell" value="'+response.jobs.fwd_sell+'">';
											my_html += '</div>';
										my_html += '</div>';
									my_html += '</div>';
								my_html += '</div>';
								...
								my_html += '<div class="row col-md-12 m-1">';
									my_html += '<div class="col-md-6">';
										my_html += '<div class="form-group">';
											my_html += '<label class="control-label">Quotation Copy</label>';
											if(response.jobs.quote_copy){
												my_html += '<div class="input-group">';
													my_html += '<div class="custom-file" data-type="fwd_quote_copy">';
														my_html += '<input type="hidden" name="fwd_quote_copy" value='+response.jobs.fwd_quote_copy+'>';
														my_html += '<a href="'+response.jobs.quote_copy+'" download class="btn btn-success btn-sm"><i class="fa fa-download"></i> Download</a>';
														my_html += '<a href="" class="ml-3 deleteAttach btn btn-danger btn-sm"><i class="icon-trash"></i> Delete</a></td>';
													my_html += '</div>';
												my_html += '</div>';
											}
											else
											{
												my_html += '<div class="input-group">';
													my_html += '<div class="custom-file">';
														my_html += '<input type="file" name="quote_copy" class="custom-file-input form-control-sm">';
														my_html += '<label class="custom-file-label" for="exampleInputFile">Choose file</label>';
													my_html += '</div>';
												my_html += '</div>';
											}
									...
									my_html += '<div class="col-md-6">';
										my_html += '<div class="form-group">';
											my_html += '<label class="control-label">Quotation Email</label>';
											if(response.jobs.quote_email){
												my_html += '<div class="input-group">';
													my_html += '<div class="custom-file" data-type="fwd_quote_email">';
														my_html += '<input type="hidden" name="fwd_quote_email" value='+response.jobs.fwd_quote_email+'>';
														my_html += '<a href="'+response.jobs.quote_email+'" download class="btn btn-success btn-sm"><i class="fa fa-download"></i> Download</a>';
														my_html += '<a href="" class="ml-3 deleteAttach btn btn-danger btn-sm"><i class="icon-trash"></i> Delete</a></td>';
													my_html += '</div>';
												my_html += '</div>';
											}
											else
											{
												my_html += '<div class="input-group">';
													my_html += '<div class="custom-file">';
														my_html += '<input type="file" name="quote_email" class="custom-file-input form-control-sm">';
														my_html += '<label class="custom-file-label" for="exampleInputFile">Choose file</label>';
													my_html += '</div>';
												my_html += '</div>';
											}
										my_html += '</div>';
									my_html += '</div>';
									my_html += '<div class="col-md-12">';
										my_html += '<div class="form-group">';
											my_html += '<label class="control-label">Remarks</label>';
											my_html += '<textarea class="form-control form-control-sm" name="fwd_remarks" rows="2">'+response.jobs.fwd_remarks+'</textarea>';
										my_html += '</div>';
									my_html += '</div>';
								my_html += '</div>';
					...
					if(my_html)
	                    $('#updateForwarder').append(my_html);
	                
	                $("#modal-forwarder").modal('show');

	                //// For Shipement sub type
					$('input[type=checkbox][id=MBL').prop('checked', false);
					$('input[type=checkbox][id=HBL').prop('checked', false);
					
					$.each(response.bl_type, function(index, value) {
						if(value === 'MBL'){
							$('input[type=checkbox][id=MBL').prop('checked', true);
				    		$('#mbl_div').removeClass('d-none');
				    		
						}
				    	if(value === 'HBL'){
				    		$('input[type=checkbox][id=HBL').prop('checked', true);
				    		$('#hbl_div').removeClass('d-none');
				    	}
				    });
	                
	                $("#loader").hide();

	                $("#frmUpdateForwarder").find('input[type=checkbox][id=MBL').change(function() {
				        ...
				        if ($(this).is(':checked')) {
				            ...
				            $('#mbl_div').addClass('d-none');
				        }
				    });
				    $("#frmUpdateForwarder").find('input[type=checkbox][id=HBL').change(function() {
                    ...
                    $('#overseas_agent').kaabar_autocomplete_full({source: '<?php echo site_url($this->_clspath.$this->_class.'/json/new_agents/id/company_name') ?>'});

                    ////// Delete Attachment
					$('#frmUpdateForwarder').on('click', 'a.deleteAttach', function(event) {
						var job_id = id;
						var param = $(this).parents('div').data('type');
						Swal.fire({
		                	title: "Are you sure?",
							text: "You want to delete this attachement..!",
							icon: "warning",
		                	...
		                	if (result.dismiss === Swal.DismissReason.cancel) {
		                		...
		                		$.ajax({
								  	url: '<?php echo site_url('export') ?>/forwarder/deleteattach',
								  	type: 'POST',
								  	data: { job_id : job_id, param:param},
								  	dataType: 'json',
								  	beforeSend: function(){
							            $("#loader").show();
							        },
								  	success:function(response) {
								  		if(response.success === true) {
								  			...
								  			Swal.fire({
										    	...
										    	$('#modal-forwarder').find('#updateForwarder').html('');
										    	updateForwarding(job_id);
										    });
										} 
										else 
										{
                    ...
                    /////// Submit Form Data
                    $("#frmUpdateForwarder").unbind('submit').on('submit', function() {
										    ...
										    if(result){
											    ...
											    for(var i=0;i < result.length;i++){
											        ...
											        if(item.id > 0) {
														...
														my_html += '<input type="hidden" class="form-control form-control-sm BillItemID" name="kbr_bill_item_id['+item.id+']" value="'+item.bill_item_id+'" title="id">';
														...
														my_html += '<input type="hidden" class="form-control form-control-sm" name="kbr_vendor_id['+item.id+']" value="'+item.vendor_id+'" title="id" required></td>'
														...
														my_html += '<td class="align-middle '+$non_stax+'">';
															...
															my_html += '<input type="hidden" class="Unchanged CurrencyNameId" value="'+item.currency_id+'" name="kbr_currency['+item.id+']" title="id">'
														my_html += '</td>';
														...
														my_html += '<td class="align-middle '+$non_stax+'">';
															...
															my_html += '<input type="hidden" class="SellCurrencyNameId Unchanged" value="'+item.sell_currency_id+'" title="id" name="kbr_sell_currency['+item.id+']">'
														my_html += '</td>';
    ...
    {
    	$.ajax({
            url: '<?php echo site_url() ?>export/jobs/getAllJob',
            type: 'post',
            dataType: 'json',
            beforeSend: function(){
	            $("#loader").show();
            ...
            success:function(response) {
			    ...
			    my_html += '<table class="table table-sm table-bordered">';
		            my_html += '<thead>';
		                my_html += '<tr>';
		                    my_html += '<th class="text-center align-middle">Job No</th>';
		                    my_html += '<th class="text-center align-middle">Date</th>';
		                    my_html += '<th class="text-center align-middle">Shipper</th>';
		                    my_html += '<th class="text-center align-middle">No Of Container</th>';
		            ...
		            my_html += '<tbody>';
		            	...
		            	if(response){
		                    ...
		                    for(var i=0;i < result.length;i++){
	                        	...
	                        	my_html += '<tr>';
	                        		my_html += '<td class="align-middle text-center">'+item.idkaabar_code+'</td>';
		                        	my_html += '<td class="align-middle text-center">'+item.date+'</td>';
		                            my_html += '<td class="align-middle text-center">'+item.shipper_name+'</td>';
		                            my_html += '<td class="text-center align-middle">'+item.cont+'</td>';
		                            my_html += '<td class="text-center align-middle"><a href="<?php echo base_url('export/job_status/index'); ?>/'+item.id+'" class="btn btn-info btn-sm" target="_blank">Status</a></td>';
								my_html += '</tr>';
							}
						}
					my_html += '</tbody>';
				...
				if(my_html)
                    $('#updateJobList').append(my_html);
                    
                $("#modal-joblist").modal('show');
                $("#loader").hide();
			}
		});
    }
    ...
    {   
        ...
        if(id){
        	$.ajax({
                url: '<?php echo site_url() ?>export/job_document/get/'+id,
                type: 'post',
                dataType: 'json',
                beforeSend: function(){
		            $("#loader").show();
                ...
                success:function(response) {
					...
					<?php //if(Auth::hasPermission(Auth::READ, ['resource/location'])){ ?>
						my_html += '<form action="<?php echo base_url('export/job_document/attach/') ?>'+id+'" method="post" enctype=" multipart/form-data" id="importDocument">';
							my_html += '<div class="modal-body">';
        ...
        if(id){
        	$.ajax({
                ...
                success:function(response) {
                    ...
                    var my_html = '';    
                    <?php //if(Auth::hasPermission(Auth::READ, ['resource/location'])){ ?>
						my_html += '<form action="<?php echo base_url('export/container/edit/') ?>'+id+'" method="post" id="frmUpdateContainers">';
							my_html += '<div class="modal-body p-0">';
								my_html += '<div class="row col-md-12 m-1">';
									my_html += '<div class="col-md-4">';
										my_html += '<div class="form-group">';
					...
					$('table.DataEntry').on('click', 'a.deleteAttach', function(event) {
						...
						Swal.fire({
		                	...
		                	if (result.dismiss === Swal.DismissReason.cancel) {
		                		...
		                		$.ajax({
								  	url: '<?php echo site_url('export') ?>/container/deleteattach',
								  	type: 'POST',
								  	data: { job_id : job_id, row_id : itemrow_id },
								  	dataType: 'json',
								  	beforeSend: function(){
    ...
    {   
    	...
    	if(job_id & id){
    		...
    		$.ajax({
                url: '<?php echo site_url() ?>export/container/edit/'+job_id+'/'+id,
                type: 'get',
                dataType: 'json',
                beforeSend: function(){
		            $("#loader").show();
                ...
                success:function(response) {
                	...
                	if(response.success == true){
                		...
                		var my_html = '';    
                		<?php //if(Auth::hasPermission(Auth::READ, ['resource/location'])){ ?>
							my_html += '<form action="<?php echo base_url('export/container/edit/') ?>'+job_id+'/'+id+'" method="post" id="frmUpdateContainer">';
								my_html += '<div class="modal-body">';
									my_html += '<div class="row">';
										my_html += '<div class="col-md-6">';
											my_html += '<div class="form-group">';
