                ...
                success:function(response) {
	     				...
	     				if(response.quo_details.length){
					    	...
					    	for (var i = 0; i < result.length; i++) {
						        quo_details += '<tr>';
									...
									quo_details += '<td><input type="text" class="form-control form-control-sm Focus" name="amount['+result[i].id+']" value="'+result[i].amount+'"></td>';
					...
					$('#Print').on('click', function(e){
						...
						if(id > 0){
							...
							Swal.fire({
								html: "Please select any Quotation for Print....!",
								icon: "error",
							});
						}
