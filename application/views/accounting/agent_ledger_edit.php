				...
				<fieldset>
					<div class="form-group<?php echo (strlen(form_error('agent_id')) > 0 ? ' has-error' : '') ?>">
						<label class="control-label">Choose Agent</label>
							...
							<input type="hidden" name="agent_id" value="<?php echo $row['agent_id'] ?>" id="AgentID" />
							<input type="text" class="form-control form-control-sm" value="<?php echo $agent_name ?>" id="ajaxAgent" />
				...
				<fieldset>
					...
					<div class="row">
						...
						<div class="col-md-4">
							...
							<label><input type="radio" name="save_to" value="G" /> <span class="label label-info">Group</span></label>
						</div>
					</div>
				</legend>	
...
$(document).ready(function() {
	$("#ajaxAgent").autocomplete({
		source: '<?php echo site_url('master/agent/ajax') ?>',
		minLength: 2,
		focus: function(event, ui) {
			$("#ajaxAgent").val( ui.item.name);
			return false;
		},
