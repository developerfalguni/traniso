...
if (isset($pdf)) : 
	...
	foreach($profit_loss as $pl) {
		if ($last_group == '') {
			if ($pl['deemed_positive'])
				$expenses .= '<tr><td class="bordertop">' . $pl['group_name'] . '</td>';
			else
				$incomes .= '<tr><td class="bordertop">' . $pl['group_name'] . '</td>';
		}
		else if ($last_group != $pl['group_name']) {
			if ($last_dp == 1) {
				$expenses .= '<td class="alignright bordertop">' . inr_format($expenses_total) . '</td></tr>';
				$expenses_total = 0;
			}
			else {
				$incomes .= '<td class="alignright bordertop">' . inr_format($incomes_total) . '</td></tr>';
				$incomes_total = 0;
			}
	...
	if ($netprofit > 0) {
		$expenses .= '<tr><td class="bold bordertop">Net Profit</td><td class="alignright bold bordertop">' . inr_format($netprofit) . '</td></tr>';
		$expenses_grand_total = bcadd($expenses_grand_total, $netprofit, 2);
	}
	else {
		$incomes  .= '<tr><td class="bordertop">Net Loss</td><td class="alignright bold bordertop">' . inr_format(abs($netprofit)) . '</td></tr>';
		$incomes_grand_total = bcadd($incomes_grand_total, abs($netprofit), 2);
	}

	$expenses .= '<tr><td class="bold bordertop borderbottom">Grand Total</td><td class="alignright bold bordertop borderbottom">' . inr_format($expenses_grand_total) . '</td></tr>';
	$incomes  .= '<tr><td class="bold bordertop borderbottom">Grand Total</td><td class="alignright bold bordertop borderbottom">' . inr_format($incomes_grand_total) . '</td></tr>';
