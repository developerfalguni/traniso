...
if (isset($pdf)) : 
	...
	foreach($balance_sheet as $side => $bl) {
		foreach ($bl as $gid => $groups) {
			foreach ($groups as $key => $value) {
				if (is_array($value)) {
					foreach ($value as $id => $account) {
						$$side .= '<tr>
			<td class="tiny">&nbsp;&nbsp;&nbsp;' . $account['name'] . '</td>
			<td class="alignright">' . inr_format($account['amount']) . '</td>
			<td>&nbsp;</td>
		</tr>';
					}
				}
					...
					$$side .= '<tr>
			<td class="bold bordertop">' . $key . '</td>
			<td class="bordertop">&nbsp;</td>
			<td class="alignright bold bordertop">' . inr_format($value) . '</td>
		</tr>';
					$total[$side] += $value;
				}
			}
	...
	$liabilities .= '<tr>
		...
		<td class="bold bordertop borderbottom">&nbsp;</td>
...
<div id="FixedToolbar">
	...
	<tr>
		...
		<td>
			<div class="form-group<?php echo (strlen(form_error('upto')) > 0 ? ' has-error' : '') ?>">
				<label class="control-label">Upto</label>
				<div class="input-group input-group-sm">
					<input type="text" class="form-control form-control-sm DatePicker" name="upto" value="<?php echo ($upto != '00-00-0000' ? $upto : '') ?>">
