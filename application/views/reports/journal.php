	...
	<tr>
		...
		<td>
			<div class="nowrap">
				...
				<div class="btn-group">
					<button data-toggle="dropdown" class="btn btn-success dropdown-toggle"><?php echo (isset($parsed_search['pending']) ? '<i class="icon-filter4"></i>' : '') ?> Pending Job <span class="caret"></span></button>
					<ul class="dropdown-menu nav-menu-scroll" id="FilterPending">
						<li><a class="red" href="javascript: filter('pending:')">Clear Filter</a></li>
						<li><a href="javascript: filter('pending:0')">Pending</a></li>
						<li><a href="javascript: filter('pending:1')">Attached</a></li>
					</ul>
				</div>
				<div class="btn-group">
					...
					<ul class="dropdown-menu nav-menu-scroll pull-right" id="FilterJobNo"></ul>
				</div>
				<div class="btn-group">
...
$i = 1;
foreach ($rows as $r) {
	foreach (explode(',', $r['job_nos']) as $j) {
		$filter['job_no'][$j] = 1;
	}
	...
	<td class="aligncenter">' . anchor('accounting/journal/edit/'.$r['voucher_book_id'].'/'.$r['id'], $r['id2_format'], 'target="_blank"') . '</td>
