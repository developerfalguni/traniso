	...
	<tr>
		...
		<td>
			<div class="nowrap">
				...
				<div class="btn-group">
					<button data-toggle="dropdown" class="btn btn-success dropdown-toggle"><?php echo (isset($parsed_search['book']) ? '<i class="icon-filter4"></i>' : '') ?> Voucher Book <span class="caret"></span></button>
					<ul class="dropdown-menu nav-menu-scroll" id="FilterBook">
						<li><a class="red" href="javascript: filter('book:')">Clear Filter</a></li>
						<li><a href="javascript: filter('book:Invoice')">Invoice</a></li>
						<li><a href="javascript: filter('book:Debit Note')">Debit Note</a></li>
					</ul>
				</div>
				<div class="btn-group">
					...
					<ul class="dropdown-menu nav-menu-scroll" id="FilterType">
						...
						<li><a href="javascript: filter('type:Import-Export')">Import-Export</a></li>
						<li><a href="javascript: filter('type:Transportation')">Transportation</a></li>
					</ul>
				</div>
				<div class="btn-group">
...
foreach ($rows as $r) {
	...
	$total['c20']        += $r['c20'];
	$total['c40']        += $r['c40'];
	$total['advance']     = bcadd($total['advance'], $r['advance'], 2);
	$total['amount']      = bcadd($total['amount'], $r['amount'], 2);
	$total['stax_amount'] = bcadd($total['stax_amount'], $r['stax_amount'], 2);
	$total['net_amount']  = bcadd($total['net_amount'], $r['net_amount'], 2);
	$total['due_amount']  = bcadd($total['due_amount'], ($r['due_amount'] - $r['advance']), 2);
	...
	<td class="aligncenter">' . $r['c20'] . '</td>
	<td class="aligncenter">' . $r['c40'] . '</td>
	<td>' . $r['debit_name'] . '</td>
	<td>' . $r['credit_name'] . '</td>
	<td class="alignright">' . inr_format($r['amount']) . '</td>
	<td class="alignright">' . inr_format($r['stax_amount']) . '</td>
	<td class="alignright">' . inr_format($r['net_amount']) . '</td>
	<td class="alignright">' . inr_format($r['due_amount'] - $r['advance']) . '</td>
</tr>';
} 
?>
</tbody>
...
<tr>
	...
	<th></th>
	<th></th>
	<th class="alignright"><?php echo inr_format($total['amount'] - $total_advance) ?></th>
	<th class="alignright"><?php echo inr_format($total['stax_amount']) ?></th>
	<th class="alignright"><?php echo inr_format($total['net_amount']) ?></th>
	<th class="alignright"><?php echo inr_format($total['due_amount']) ?></th>
</tr>
</tfoot>
</table>
