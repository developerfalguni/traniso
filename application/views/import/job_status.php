...
<script type="text/javascript">
    ...
    {   
        ...
        if(job_id){
        	$.ajax({
                url: '<?php echo site_url() ?>import/job_status/get/'+job_id,
                type: 'post',
                dataType: 'json',
                beforeSend: function(){
		            $("#loader").show();
     				...
     				if(response.containers.length){
				    	...
				    	for (var i = 0; i < result.length; i++) {
					        costsheetList += '<div class="row m-0">';
								...
								costsheetList += '</div>';
								costsheetList += '<div class="col-lg-4 border-right border-bottom">';
									costsheetList += '<span class="small">'+response.vendor_name+'</span>';
								costsheetList += '</div>';
								costsheetList += '<div class="col-lg-4 border-bottom">';
									costsheetList += '<span class="small"><a href="'+result[i].file+'" download><i class="fa fa-download fa-lg text-success"></i></a></span>';
								costsheetList += '</div>';
							costsheetList += '</div>';
						}
					}
				    	...
				    	for (var i = 0; i < result.length; i++) {
					        uploadDocumentList += '<div class="row m-0">';
								...
								uploadDocumentList += '<div class="col-lg-4 border-right border-bottom">';
									uploadDocumentList += '<span class="small">'+result[i].name+'</span>';
								uploadDocumentList += '</div>';
								uploadDocumentList += '<div class="col-lg-2 border-right border-bottom">';
