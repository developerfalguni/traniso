<?php
	echo form_open($this->_clspath.$this->_class.'/ajaxEdit/'.$type, 'id="MainForm"');
	echo form_hidden($id);
?>
		...
		<div class="row">
			<div class="col-md-6">
				<div class="form-group row">
					...
					<div class="col-sm-9">
						<?php echo form_dropdown('invoice_list', [], '', 'class="form-control form-control-sm mb-0" id="importInvoiceSelect2" '); ?>
					</div>
				</div>
			</div>
			<div class="col-lg-6">
					        ...
					        <table class="table table-sm table-bordered DataEntry">
					            <thead>
					            	<tr>
					                    ...
					                    <th class="text-center align-middle">Gst</th>
					                    ...
					                    <th class="text-center align-middle">Total GST</th>
        	...
        	$.ajax({
                ...
                success:function(response) {
					...
					$('.DataEntry').on('keyup', '.ExchangeRate', function() {
						...
						var new_amount = amount + gst_amount;
					...
					$('.DataEntry').on('change', '.GST', function() {
						var ex_rate = $(this).parents('tr').children('td').find('.ExchangeRate').val();
						var rate    = $(this).parents('tr').children('td').find('.Rate').val();
						var is_inr  = $(this).parents('tr').children('td').find('.IsINR').val();
						var unit    = $(this).parents('tr').children('td').find('.Units').val();
					...
					function getTotal(){
					    ...
					    $(".NetAmount").each(function(){
					        total_net_amount += +$(this).val();

					        var add_charge = $("#AdditionalCharge").val();
						    if(!add_charge)
						    	{
						    		$("#NetAmount").val(total_net_amount.toFixed(2));	
						    	}else{
