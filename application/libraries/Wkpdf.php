...
class WKPDF {
	...
	private $pdf='';
	private $status='';
	private $margin_top='10mm';
	private $margin_bottom='10mm';
	private $margin_left='10mm';
	private $margin_right='10mm';
...
// 			elseif(`grep -i intel /proc/cpuinfo`!='')	self::$cpu='i386';
			$processor = trim(`uname -p`);
			if($processor == 'x86_64') 	self::$cpu='amd64';
			elseif($processor == 'i686')	self::$cpu='i386';
			elseif($processor == 'unknown') {
				$processor = trim(`uname -m`);
				if($processor == 'x86_64') 	self::$cpu='amd64';
				elseif($processor == 'i686')	self::$cpu='i386';
				elseif($processor == 'armv7l')	self::$cpu='armv7l';
				else throw new Exception("WKPDF couldn't determine CPU ($processor)");
			}
		}
		return self::$cpu;
	}
	...
	public function __construct() {
		$GLOBALS['WKPDF_BASE_PATH'] = FCPATH;

		if (defined('WINDOWS'))
			$this->cmd="C:\\Progra~1\\wkhtmltopdf\\wkhtmltopdf.exe";
		else
	 ...
	 * By default page margin is 10mm.
	 * @param string $top_margin Formal paper top_margin (eg; 10mm...)
	 */
	 ...
	 * Set page margin size.
	 * By default page margin is 10mm.
	 * @param string $top_margin Formal paper bottom_margin (eg; 10mm...)
	 */
	...
	public function render() {
		...
		if (defined('WINDOWS')) {
			$web=$GLOBALS['WKPDF_BASE_PATH'].'tmp\\'.basename($this->tmp);
			ob_start();
			$this->pdf['stderr']=passthru(
				$this->cmd
				.(($this->copies>1)?' --copies '.$this->copies:'')           // number of copies
				.' --margin-top '.$this->margin_top                          // margin-top
				.' --margin-bottom '.$this->margin_bottom                    // margin-bottom
				.' --margin-left '.$this->margin_left                        // margin-left
				.' --margin-right '.$this->margin_right                      // margin-right
			...
			$this->pdf['return']);
			$this->pdf['stdout']=ob_get_contents();
			ob_end_clean();
		}
		else {
			$web=$GLOBALS['WKPDF_BASE_PATH'].'tmp/'.basename($this->tmp);
