...
class Voucher extends MY_Controller {
	...
	function show($voucher_book_id, $id2, $id3, $company_id) {
		if ($this->_company['id'] == $company_id)
			redirect($this->_clspath.$this->_class."/edit/$voucher_book_id/$id2/$id3");
		...
		if (($id2 == 0 OR $id3 == 0) && $voucher_book['default_ledger_id'] > 0 && $voucher_book['dr_cr'] == 'Dr')
			$dr_ledger = $this->kaabar->getRow('ledgers', $voucher_book['default_ledger_id']);
		else if (($id2 == 0 OR $id3 == 0) && $voucher_book['default_ledger_id'] > 0 && $voucher_book['dr_cr'] == 'Cr')
			$cr_ledger = $this->kaabar->getRow('ledgers', $voucher_book['default_ledger_id']);
		
		$row = $this->accounting->getVoucher($voucher_book_id, $id2, $id3, $company_id);
		if (isset($dr_ledger)) {
		...
		$data['focus_id'] = ($id2 == 0 && $id3 == 0 ? 'Date' : 'DebitAccount');
		$data['page_title'] = $this->accounting->getBookName($voucher_book_id) . ' ' . humanize($this->_class);

		switch ($voucher_book['voucher_type_id']) {
			case 1:	// Contra
			case 5: // Journal
				$data['sub_vouchers'] = $this->accounting->getSubVouchers($voucher_book_id, $id2);
				break;
			case 2: // Credit Note
			case 3: // Debit Note
			case 4: // Invoice
	...
	function edit($voucher_book_id, $id2 = 0, $id3 = 0, $prev_id3 = 0) {
		$voucher_book_id = intval($voucher_book_id);
		$id2 = intval($id2);
		$id3 = intval($id3);
		...
		$this->form_validation->set_rules('date', 'Date', 'trim|required|min_length[10]|callback__checkDate['.$voucher_book_id.','.$id2.']');
		...
		if ($id2 > 0 && $id3 == 0) {
			$row = $this->accounting->getVoucher($voucher_book_id, $id2, 1);
			$row['id']                  = 0;
			$row['dr_ledger_id']        = 0;
			$row['debit_account']       = '';
		...
		if ($id2 > 0 && $id3 == 0 && $prev_id3 > 0) {
			$row = $this->accounting->getVoucher($voucher_book_id, $id2, $prev_id3);
			$row['id']                  = 0;
			$row['dr_ledger_id']        = 0;
			$row['debit_account']       = '';
			$row['dr_tds_class_id']  	= 0;
			...
			$row = $this->accounting->getVoucher($voucher_book_id, $id2, $id3);
		...
		$this->form_validation->set_rules('invoice_no', 'Bill No', 'trim|callback__is_duplicatebill['.$voucher_book_id.','.$id2.']');
		...
		if ($this->form_validation->run() == false) {
			...
			switch ($voucher_book['voucher_type_id']) {
				case 1:	// Contra
				case 5: // Journal
					$data['page'] = $this->_clspath.'journal_edit';
					$data['sub_vouchers'] = $this->accounting->getSubVouchers($voucher_book_id, $id2);
					break;
				case 2: // Credit Note
				case 3: // Debit Note
				case 4: // Invoice
					...
					$data['page'] = $this->_clspath.'invoice_edit';
					break;
				case 7: // Payment
					$this->load->helper("datefn");
					$data['page'] = $this->_clspath.'payment_edit';
					$data['sub_vouchers'] = $this->accounting->getSubVouchers($voucher_book_id, $id2);
					$data['months'] = getMonthsInBetween($row['date'], date('01-m-Y', strtotime('-12 month', strtotime($row['date']))));
					break;
				case 11: // Receipt
					$data['page'] = $this->_clspath.'receipt_edit'; 
					$data['sub_vouchers'] = $this->accounting->getSubVouchers($voucher_book_id, $id2);
					break;
				default:
			...
			if ($id3 == 0 && isset($data['sub_vouchers']) && count($data['sub_vouchers']) > 0)
			...
			checkDuplicateFormSubmit($this->_clspath.$this->_class.'/edit/'.$voucher_book_id.'/'.$id2.'/'.$id3);
			...
			switch ($voucher_book['voucher_type_id']) {
				...
				case 5: // Journal
				case 7: // Payment
					$this->_updateJournal($id);  break;
				case 2: // Credit Note
				case 3: // Debit Note
				case 4: // Invoice
					$this->_updateInvoice($voucher_book_id, $id);
				case 11: // Receipt
					$this->_updateReceipt($id);  break;
				default:
					$this->_updateVoucher($id);
			}
			setSessionAlert('Changes saved successfully', 'success');

			redirect($this->_clspath.$this->_class.'/edit/'.$voucher_book_id.'/'.$data['id2'].'/'.$data['id3']);
		}
	}
	...
	function _is_duplicatebill($invoice_no, $args) {
		...
		if($voucher['id'] == 0 || trim(strlen($invoice_no)) == 0)
			...
			$this->form_validation->set_message('_is_duplicatebill', 'Duplicate Bill No found with Voucher No. ' . anchor($this->_clspath.$this->_class."/edit/".$voucher['voucher_book_id'].'/'.$voucher['id2'].'/'.$voucher['id3'], $voucher['id2'].'/'.$voucher['id3']));
			return FALSE;
		}
	}
	...
	function ajaxBL($vessel_ledger_id = 0) {
		if ($this->input->is_ajax_request()) {
			...
			$sql = "SELECT J.id, LEFT(J.type, 1) AS type, J.bl_no, P.name AS party, 
	...
	function ajaxInvoiceBL() {
		if ($this->input->is_ajax_request()) {
			...
			WHERE LENGTH(J.bl_no) > 0 AND (J.bl_no LIKE '%$search%' OR P.name LIKE '%$search%')
			ORDER BY bl_no
			LIMIT 0, 50";
			$rows = $this->db->query($sql)->result_array();
			header('Content-type: application/json; charset=utf-8');
