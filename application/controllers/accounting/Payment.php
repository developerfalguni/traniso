...
class Payment extends MY_Controller {
	...
	function index($voucher_book_id = 0, $starting_row = 0) {
		...
		$config['total_rows']  = $this->_count($voucher_book_id, $search);
		$config['per_page']    = Settings::get('rows_per_page');
		$this->pagination->initialize($config);
		
		$data['list']['data'] = $this->_get($voucher_book_id, $search, $starting_row, $config['per_page']);
	...
	function edit($voucher_book_id, $id = 0, $prev_id = 0) {
		...
		if ($this->form_validation->run() == false) {
			...
			if ($id == 0 && isset($data['sub_vouchers']) && count($data['sub_vouchers']) > 0)
			...
			switch ($row['category']) {
				case 'Journal':
					$data['journals'] = $this->_getJournalVouchers($row['dr_ledger_id'], $row['id'], TRUE);
					break;
				case 'Invoices':
					$data['invoices'] = $this->_getInvoices($row['dr_ledger_id'], $row['id'], TRUE);
					break;
				case 'Pumps':
					$data['pumps'] = $this->_getPumps($row['dr_ledger_id'], $row['id'], TRUE);
					break;
				case 'Trips':
			...
			if (Auth::hasAccess($id > 0 ? Auth::UPDATE : Auth::CREATE)) {
				...
				if ($row['audited'] == 'No') {
					...
					$this->_updatePayment($id);

					
					$paid_amounts = $this->input->post('paid_amount');
					if ($paid_amounts != null && $data['category'] == 'Journal') {
						foreach ($paid_amounts as $index => $amount) {
							...
							$total_paid = $query->row_array();

							$query = $this->db->query("SELECT V.id, V.amount AS due_amount
							FROM vouchers V 
							WHERE V.id = ?", array($total_paid['voucher_id2']));
							$total_journal = $query->row_array();
							$balance = bcsub($total_journal['due_amount'], $total_paid['amount']);

							if ($balance <= 0) {
								$this->db->update($this->_table, array('status' => 'Paid'), array('id' => $total_paid['voucher_id2']));
							}
							else {
								$this->db->update($this->_table, array('status' => 'Pending'), array('id' => $total_paid['voucher_id2']));
							}
						}
					}
					else if ($paid_amounts != null && $data['category'] == 'Trips') {
						foreach ($paid_amounts as $index => $amount) {
							$this->kaabar->save('voucher_trips', array('advance' => $amount), array('id' => $index));
						}
					}
	...
	function _count($voucher_book_id = 0, $search = '') {
		$company_id = $this->accounting->getCompanyID();
		$years = explode('_', $this->accounting->getFinancialYear());
	...
	function _get($voucher_book_id = 0, $search = '', $offset = 0, $limit = 25) {
		$company_id = $this->accounting->getCompanyID();
		$years = explode('_', $this->accounting->getFinancialYear());
		$from_date = $years[0] . '-04-01';
		$to_date   = $years[1] . '-03-31';
	...
	function journals($party_id, $id) {
		if ($this->input->post('voucher_id')) {

			$query = $this->db->query("SELECT VV.voucher_id2 FROM voucher_vouchers VV WHERE VV.voucher_id = ?", array($id));
			$rows = $query->result_array();

			$this->db->delete('voucher_vouchers', array('voucher_id' => $id));

			foreach ($rows as $row) {
				$query = $this->db->query("SELECT V.id, V.amount AS due_amount FROM vouchers V WHERE V.id = ?", array($row['voucher_id2']));
				$total_journal = $query->row_array();
				$balance = bcsub($total_journal['due_amount'], 0);

				if ($balance <= 0) {
					$this->db->update($this->_table, array('status' => 'Paid'), array('id' => $row['voucher_id2']));
				}
				else {
					$this->db->update($this->_table, array('status' => 'Pending'), array('id' => $row['voucher_id2']));
				}
			}

			$this->db->delete('voucher_vouchers', array('voucher_id' => $id));

			$voucher_id2 = $this->input->post('voucher_id2');
			if ($voucher_id2 != null) {
				$paid_amounts = $this->input->post('paid_amount');
				foreach ($voucher_id2 as $index => $voucher_id2) {
					if (intval($voucher_id2) > 0) {
						$row = array(
							'voucher_id'  => $id,
							'voucher_id2' => $voucher_id2,
							'amount'      => $paid_amounts[$index],
						);
		...
		$data['rows'] = $this->_getJournalVouchers($party_id, $id);

		$data['javascript']  = array('js/jquery.filtertable.min.js');
		$data['voucher_id']  = array('voucher_id' => $id);

		$data['id']          = array('id' => $id);
		$data['page_title']  = 'Journal Vouchers';
		$data['hide_title']  = true;
		$data['hide_menu']   = true;
		$data['hide_footer'] = true;
		$data['page']        = $this->_clspath.$this->_class.'_journal';
		$this->load->view('index', $data);
	}

	function _getJournalVouchers($party_id, $id, $attached_only = FALSE) {
		$attached = ($attached_only ? '' : 'ISNULL(VV.id) OR V.status = "Pending" OR ');
		$sql = "SELECT V.id, VV.id AS voucher_id2, V.voucher_book_id, V.id2, 
			CONCAT(VT.name,'/edit/',VB.id,'/',V.id,'/',V.id3) AS url, V.id2_format, 
			DATE_FORMAT(V.date, '%d-%m-%Y') AS voucher_date, V.invoice_no, DATE_FORMAT(V.invoice_date, '%d-%m-%Y') AS invoice_date,
			V.amount AS due_amount, COALESCE(VV.amount, 0) AS paid_amount, COALESCE(TDS.tds_amount, 0) AS tds_amount,
			(V.amount - (COALESCE(TDS.tds_amount, 0) + SUM(COALESCE(VV.amount, 0)))) AS balance
		FROM vouchers V 
			INNER JOIN voucher_books VB ON (VB.company_id = ? AND V.cr_ledger_id = ? AND V.voucher_book_id = VB.id)
			INNER JOIN voucher_types VT ON (VB.voucher_type_id = VT.id AND VT.name = 'Journal')
			LEFT OUTER JOIN voucher_vouchers VV ON VV.voucher_id2 = V.id
			LEFT OUTER JOIN vouchers TDS ON (V.voucher_book_id = TDS.voucher_book_id AND V.id2 = TDS.id2 AND TDS.tds_amount > 0 AND V.amount = TDS.invoice_amount)
		WHERE ($attached VV.voucher_id = ?)
		GROUP BY V.id
		ORDER BY ISNULL(VV.id), V.date";
		$query = $this->db->query($sql, array($this->accounting->getCompanyID(), $party_id, $id));
		return $query->result_array();
	}


	function invoices($party_id, $id) {
		if ($this->input->post('voucher_id')) {
			$this->db->delete('voucher_vouchers', array('voucher_id' => $id));
		...
		$data['rows'] = $this->_getInvoices($party_id, $id);
		...
		$data['hide_menu']   = true;
		$data['hide_footer'] = true;
		$data['page']        = $this->_clspath.$this->_class.'_invoice';
		$this->load->view('index', $data);
	}

	function _getInvoices($party_id, $id, $attached_only = FALSE) {
		$attached = ($attached_only ? '' : 'ISNULL(VV.id) OR ');
		$sql = "SELECT V.id, VV.id AS voucher_id2, V.voucher_book_id, V.id2, 
			CONCAT(VT.name,'/edit/',VB.id,'/',V.id,'/',V.id3) AS url, V.id2_format, 
			DATE_FORMAT(V.date, '%d-%m-%Y') AS voucher_date, V.invoice_no, DATE_FORMAT(V.invoice_date, '%d-%m-%Y') AS invoice_date,
			V.amount AS due_amount, COALESCE(VV.amount, 0) AS paid_amount, (V.amount - SUM(COALESCE(VV.amount, 0))) AS balance
		FROM vouchers V 
			INNER JOIN voucher_books VB ON (VB.company_id = ? AND V.cr_ledger_id = ? AND V.voucher_book_id = VB.id)
	...
	function _getTrips($dr_ledger_id, $id, $attached_only = FALSE) {
		...
		$sql = "SELECT VT.id, VT.voucher_id, J.id2_format AS job_no, VT.advance AS paid_amount, T.id AS trip_id, 
	...
	function preview($voucher_book_id, $id, $pdf = 0, $sub_voucher = 0, $letterhead = 0) {
		...
		switch ($data['voucher']['category']) {
			case 'Journal':
				$data['journals'] = $this->_getJournalVouchers($data['voucher']['dr_ledger_id'], $data['voucher']['id'], TRUE);
				break;
			case 'Invoices':
