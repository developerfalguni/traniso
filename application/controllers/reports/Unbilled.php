...
class Unbilled extends MY_Controller {
	var $_fields;
	...
	function _get($from_date, $to_date, $search, $parsed_search) {
		$sql = "SELECT J.id AS job_id, J.type, J.id2_format, P.name AS party_name, 
			COUNT(IF(CT.size=20, C.id, NULL)) AS container_20, COUNT(IF(CT.size=20, NULL, C.id)) AS container_40
		...
		if (count($result) > 0) {
			...
			$rows = $query->result_array();
			foreach ($rows as $row) {
				$result[$row['job_id']]['transportation'] = array('no' => $row['transportation_no'], 'amount' => $row['amount']);
			}
			...
			FROM voucher_details VD 
				INNER JOIN vouchers V ON VD.voucher_id = V.id
				INNER JOIN voucher_books VB ON V.voucher_book_id = VB.id
				INNER JOIN voucher_types VT ON (VB.voucher_type_id = VT.id AND VT.name = 'Payment')
			WHERE VD.job_id IN ($job_ids)
			GROUP BY VD.job_id";
			$query = $this->db->query($sql);
			$rows = $query->result_array();
