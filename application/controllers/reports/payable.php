...
class Payable extends MY_Controller {
	...
	function index() {
		...
		$data['page_title'] = "Payable Report";
	...
	function _get($from_date, $to_date, $search) {
		...
		// Sundry Creditor 16
		$sql = "SELECT V.id, CONCAT(VT.name,'/edit/',VB.id,'/',V.id) AS url, V.id2, V.id3, V.id2_format, V.invoice_no,
			GROUP_CONCAT(DISTINCT VD.id2_format SEPARATOR ', ') AS job_no,
			...
			V.amount, COALESCE(TDS.tds_amount, 0) AS tds_amount, 
			SUM(COALESCE(VV.amount, 0)) AS paid_amount,
			(V.amount - (COALESCE(TDS.tds_amount, 0) + SUM(COALESCE(VV.amount, 0)))) AS balance_amount
		FROM vouchers V 
			...
			INNER JOIN account_groups AG ON (CL.account_group_id = AG.id AND AG.account_id = 16)
			LEFT OUTER JOIN vouchers TDS ON (V.voucher_book_id = TDS.voucher_book_id AND V.id2 = TDS.id2 AND TDS.tds_amount > 0)
			...
			LEFT OUTER JOIN (
				SELECT VD.voucher_id, VD.job_id, J.id2_format
				FROM voucher_details VD LEFT OUTER JOIN jobs J ON VD.job_id = J.id
				GROUP BY VD.voucher_id
			) VD ON V.id = VD.voucher_id
	...
	function preview($pdf = 0) {
		...
		$data['page_title'] = 'Bills Payables';
		$data['page_desc'] = "For the Period $from_date - $to_date";

		if ($pdf) {
			$filename = $data['page_title'];
			$html = $this->load->view($this->_clspath.'/payable_preview', $data, true);
			$pdf = new Pdf(array(
				'no-outline',
			...
			$this->load->view($this->_clspath.'/payable_preview', $data);
		}
	}
