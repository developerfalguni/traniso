...
class Export_quote extends MY_Controller {
	...
	function getQuote($quote_id = 0){
		...
		else
		{
			...
			$response['messages'] = 'Please select Quotation';
		}
	...
	function old_edit($type = 'Export' , $id = 0) {
	...
	function print($id = 0, $pdf = 0) {
        ...
        else
        {
			...
			$Gst_invoice=$this->load->model('Gst_invoice');
