...
class Attached_document extends MY_Controller {
	...
	function od_index($job_id = 0, $child_job_id = 0, $id = 0, $md5 = '', $page_no = 1) {
		...
		$data['documents']    = $this->export->getAttachedDocs($job_id, $child_job_id, 1, 0);
	...
	function edit($job_id = 0, $child_job_id = 0, $id = 0, $page_no = 1) {
		...
		$data['id']               = array('id' => $id);
		$data['jobs']             = $this->export->getJobsInfo($job_id, true, site_url($this->_clspath));
		$data['documents']        = $this->export->getAttachedDocs($job_id, $child_job_id, 0, 1);
		$data['document']         = $this->export->getAttachedDoc($job_id, $id);
		...
		if ($document_ids != null && Auth::hasAccess(Auth::UPDATE)) {
			...
			// Reload changes
			$data['documents']  = $this->export->getAttachedDocs($job_id, 0, 1);
			$data['document'] 	= $this->export->getAttachedDoc($job_id, $id);
			$data['document']['url'] = base_url($this->_folder . $docdir . $data['document']['file']);
		}
	...
	function detach($job_id = 0, $id = 0) {
		if (Auth::hasAccess(Auth::DELETE)) {
			...
			$docdir   = $this->export->getDocFolder($this->_path, $job_id);
			$document = $this->export->getAttachedDoc($job_id, $id);
	...
	function email($job_id, $id) {
		...
		if (count($to) > 0) {
			...
			$docdir   = $this->export->getDocFolder($this->_path, $job_id);
			$document = $this->export->getAttachedDocument($job_id, $id);
			if ($this->kaabar->getField($this->_table, $document['file'], 'file', 'id') == false) {
