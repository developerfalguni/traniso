...
class Staff extends MY_Controller {
	...
	function getStaff($staff_id = 0){

		$response = [];
		
		if($staff_id > 0){

			$row = $this->kaabar->getRow('staffs', $staff_id);
		...
		else
		{
			...
			$response['messages'] = 'Plaese select Staff';
		}
		...
		if ($this->form_validation->run() == false) {
			...
			if (Auth::hasAccess(Auth::CREATE | Auth::UPDATE)) {
				$data = array(
					...
					'traces_name'        => $this->input->post('traces_name'),
	...
	function staffList() {
		...
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->db->select('id, firstname as text')
							->limit(10)
							->get('staffs');
							
			$json = $query->result_array();
			
			// $json[] = ['id' => 0, 'text' => 'New Staff'];
