...
class Non_gst extends MY_Controller {
	...
	function edit_old($type = 'Import' , $id = 0) {
		...
		if ($this->form_validation->run() == false) {
			...
			if (Auth::hasAccess($id > 0 ? Auth::UPDATE : Auth::CREATE)) {
				...
				if($row['idkaabar'] == 0)
					$this->kaabar->createKaabarNo('invoices', $id, 'INV/EXP');
	...
	function print($id = 0, $pdf = 0) {
		...
		$filename = strtoupper('Non GST Bill - '.$data['invoice']['idkaabar_code']);
        $data['page_title'] = 'Tax Invoice';

        if($pdf > 0)
        {
        	$pdf = new MYPDF('P', 'mm', 'A4', true, 'UTF-8', false);
			$pdf->SetTitle(strtoupper('Non GST Bill - '.$data['invoice']['idkaabar_code']));
		...
		if ($this->input->is_ajax_request()) {
			if($id){
				...
				if($result = 0)
				{
					...
					$response['msg'] = 'Non GST Billing not Found';			
				}
				else
				{
					...
					$response['msg'] = 'Successfully Deleted Non GST Billing';			
				}
			}
			else
			{
				$response['status'] = 'error';
				$response['msg'] = 'Please Select Non GST Billing First then delete';		
			}
			header('Content-type: application/json; charset=utf-8');
			echo json_encode($response, JSON_UNESCAPED_UNICODE);
		}
   ...
   function invoiceList() {
		...
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->db->select('id,idkaabar_code as text')
							->where('vType', 'Import')
							->where('invoice_type', 'ImportNonGst')
							->limit(10)
							->get('invoices');
			$json = $query->result_array();
