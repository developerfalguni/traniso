...
class Non_gst extends MY_Controller {
	...
	function getPendingJobsItems($job_id) {
		...
		WHERE VJD.job_id = ? AND VJD.status = 'Pending' AND VJD.billing_type = 'NG'
		...
		if($job_id){
			...
			if($rows){
				foreach ($rows as $key => $value) {
					...
					$gross_amount = $value['sell_amount'];
			...
			if (Auth::hasAccess($row_id > 0 ? Auth::UPDATE : Auth::CREATE)) {
				...
				else
				{
					...
					if($id){
						...
						if($row_id == 0){
							$this->kaabar->createKaabarNo('invoices', $id, 'TLNG');
						}
	...
	function _updateInvDetails($invoice_id, $type) {
		...
		if ($delete_ids != null OR $delete_ids != '') {
			$job_item_id	= $this->input->post('job_item_id');
			$invoice_item_id	= $this->input->post('invoice_item_id');
			
			foreach ($delete_ids as $index => $tmp) {

				if(is_array($tmp) && array_key_exists($tmp, $job_item_id) AND array_key_exists($tmp, $invoice_item_id)){
					$this->kaabar->save('costsheets', ['status' => 'Pending', 'invoice_item_id' => NULL], ['id' => $job_item_id[$index]]);
				}

				if(is_array($tmp) && array_key_exists($tmp, $invoice_item_id)){
	...
	function pdf($voucher_book_id = 1, $id = 0, $letterhead = 1, $all_bl_containers = 0) {
		...
		if($invoice['ledger_category'] == 'PARTY'){
			...
			$invoice['debit_party_address'] = $invoice['ledger']['address'].' '.$invoice['ledger']['city_id'].' '.$invoice['ledger']['pincode'].' '.$state['name'].' '.$state['gst'];
		}
		elseif($invoice['ledger_category'] == 'AGENT'){
		...
		if ($voucher_book['job_type'] != 'N/A' && $data['voucher']['job_id'] > 0) {
			...
			if($job['shipper_category'] == 'PARTY'){
				...
				$job['shipper_address'] = $job['shipper']['address'].' '.$job['shipper']['city_id'].' '.$job['shipper']['pincode'].' '.$jstate['name'].' '.$jstate['gst'];
			}
	...
	function print($id = 0, $pdf = 0) {
		...
		$filename = strtoupper('Invoice - '.$data['invoice']['idkaabar_code']);
        $data['page_title'] = 'Tax Invoice';

        if($pdf > 0)
        {
        	$pdf = new MYPDF('P', 'mm', 'A4', true, 'UTF-8', false);
			$pdf->SetTitle(strtoupper('Invoice - '.$data['invoice']['idkaabar_code']));
			$pdf->setPrintHeader(FALSE);
		...
		if ($this->input->is_ajax_request()) {
			if($id){
				...
				if($result == 0)
				{
					...
					$response['msg'] = 'Billing not Found';			
				}
				else
				{
					$einv = $this->kaabar->getRow('einvoices', ['voucher_id' => $row_id]);
					...
					else
					{
						...
						$response['msg'] = 'Invoice Successfully Deleted';	
					}
				}
			}
			else
			{
				$response['status'] = 'error';
				$response['msg'] = 'Please Select Invoice First then delete';		
			}
			header('Content-type: application/json; charset=utf-8');
			echo json_encode($response, JSON_UNESCAPED_UNICODE);
		}
    ...
    function invoiceList() {
		...
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->db->select('id,idkaabar_code as text')
							->where('vType', 'Export')
							->where('invoice_type', 'ExportNonGst')
							->limit(10)
							->order_by('id', 'DESC')
							->get('invoices');
			$json = $query->result_array();
