...
class Forex extends MY_Controller {
	...
	function getPendingJobsItems($job_id) {
		...
		WHERE VJD.job_id = ? AND VJD.status = 'Pending' AND VJD.billing_type = 'FX'
			...
			if (Auth::hasAccess($row_id > 0 ? Auth::UPDATE : Auth::CREATE)) {
				...
				else
				{
					...
					if($id){
						...
						if($row_id == 0){
							$this->kaabar->createKaabarNo('invoices', $id, 'TLFX');
						}
		...
		if ($this->input->is_ajax_request()) {
			if($id){
				...
				if($result == 0)
				{
					...
					$response['msg'] = 'Invoice not Found';			
				}
				else
				{
					$einv = $this->kaabar->getRow('einvoices', ['voucher_id' => $row_id]);
    ...
    function invoiceList() {
		...
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->db->select('id,idkaabar_code as text')
							->where('vType', 'Export')
							->where('invoice_type', 'ExportForex')
							->limit(10)
							->order_by('id', 'DESC')
							->get('invoices');
			$json = $query->result_array();
